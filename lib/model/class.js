...
module.exports = SuperJS.Api.Model.extend({
  ...
  find: function(resolve, reject, criteria) {
    ...
    //todo: check select for valid attributes
        ...
        if( typeof this.adapter.models[joinModel] === 'object' ) {
          ...
          if( typeof this.relations[join].via === 'string' ) {
            ...
            if (joins[join].select) {
              //todo: make sure selected attributes are allowed
  ...
  joinMany: function(resolve, reject, joins, records) {
          ...
          if (typeof this.relations[join].via === 'string') {
            ...
            if (typeof this.relations[join].through === 'string') {
              ...
              if( typeof this.adapter.models[joinModel].relations === 'object' &&
                ...
                joinQueries[join] = {query: query, left: this.relations[join], right: this.adapter.models[joinModel].relations[this.name] };
    ...
    if( Object.keys(joinQueries).length > 0 ) {
      ...
      for( var query in joinQueries ) {
        promises[query] = joinQueries[query].query;
      }

      Promise.props(promises)
        .then(function(results) {

          self.log.object(results);
          ...
          for( var join in results ) {
            ...
            results[join].map(function(joinRecord) {
              ...
              if( typeof identityMap[joinRecord['related::id']][join] !== 'object' ) {
                identityMap[joinRecord['related::id']][join] = [];
              }
    ...
    for( var attribute in filter ) {
      ...
      if( typeof filter[attribute] === 'object' && _.isArray(filter[attribute]) ) {
        ...
        for( var condition in filter[attribute] ) {
          ...
          var filterResult = this.filter(query, table, filter[attribute][condition], true)
