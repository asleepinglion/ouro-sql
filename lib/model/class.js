...
module.exports = SuperJS.Api.Model.extend({
  ...
  init: function(app, adapter, name) {
    ...
    if( !this.meta || !this.meta.attributes ) {
      throw new SuperJS.Error('invalid_model', 'The `' + this.name + '` model is missing meta data -- maybe your missing the _metaFile hook?');
    }
    ...
    for( var join in joins ) {
      ...
      if( typeof this.meta.relations[join].through !== 'string' && typeof this.meta.relations[join].via === 'string') {
        ...
        if (_.contains(this.fields, this.meta.relations[join].via) ) {
          ...
          return new SuperJS.Error('invalid_join', 'The `' + this.meta.relations[join].via + '` via attribute for this one-to-one join was not found on the `' + this.name + '` model.');
        }
