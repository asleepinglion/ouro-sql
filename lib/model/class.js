...
module.exports = Ouro.Api.Model.extend({
  ...
  init: function(app, adapter, name) {
    ...
    if( !this.meta || !this.meta.attributes ) {
      throw new Ouro.Error('invalid_model', 'The `' + this.name + '` model is missing meta data -- maybe your missing the _metaFile hook?');
    }

    //store fields for easy access
    this.fields = Object.keys(this.meta.attributes);
  ...
  find: function(resolve, reject, criteria) {
    ...
    if( !_.isArray(criteria.select) || Object.keys(criteria.select).length === 0 ) {
      ...
      if( badFields.length > 0 ) {
        ...
        return reject(new Ouro.Error('invalid_attribute', 'There are invalid attributes in your select list for the ' + this.name + ' model.', {details: details}));
      }
    ...
    if( ( typeof criteria.sort === 'object' || typeof criteria.sort === 'string' ) && criteria.sort.length > 0 ) {
      ...
      if( this.fields.indexOf(criteria.sort[0]) === -1 ) {
        return reject(new Ouro.Error('invalid_attribute', 'The attribute used in your sort criteria  for the ' + this.name + ' model is not valid.'));
      }
  ...
  update: function(resolve, reject, criteria, attributes, limit) {
    ...
    if( typeof attributes !== 'object' ) {
      return reject(new Ouro.Error('bad_request', 'You must provide attributes to update.'));
    }
    ...
    if( typeof criteria !== 'object' || Object.keys(criteria).length === 0 ) {
      return reject(new Ouro.Error('bad_request', 'You must provide some criteria in order to update records.'));
    }
            ...
            self.db
              ...
              .then(function(records) {
                ...
                if( records.length > 0 ) {
                  ...
                  return reject(new Ouro.Error('record_not_found', 'The id specified was not found in the ' + self.name + ' database.'));
                }
  ...
  delete: function(resolve, reject, criteria) {
    ...
    if( typeof criteria !== 'object' || Object.keys(criteria).length === 0 ) {
      return reject(new Ouro.Error('bad_request', 'You must provide some criteria in order to delete records.'));
    }
  ...
  joinOne: function(query, joins) {
    ...
    for( var join in joins ) {
      ...
      if( typeof this.meta.relations[join].through !== 'string' && typeof this.meta.relations[join].via === 'string') {

        //make sure the relationship is setup
        if( typeof this.meta.relations[join] !== 'object' ) {
          return new Ouro.Error('invalid_join', 'The `' + join + '` relationship has not been defined.');
        }

        var joinModel = (typeof this.meta.relations[join].model === 'string' ) ? this.meta.relations[join].model : join;

        //make sure the model exists
        if( typeof this.adapter.models[joinModel] !== 'object' ) {
          return new Ouro.Error('invalid_join', 'The `' + joinModel + '` requested model has not be defined.');
        }
        ...
        if (joins[join].select) {
          ...
          if( badFields.length > 0 ) {
            ...
            return new Ouro.Error('invalid_attribute', 'There are invalid attributes in your `' + join + '` join select list.', {details: details});
          }
        ...
        if (_.contains(this.fields, this.meta.relations[join].via) ) {

          //set the join type
          joins[join]._type = 'oneToOne';

          query.select(joinFields);
          query.leftOuterJoin(joinModel, this.name + '.' + this.meta.relations[join].via, '=', joinModel + '.id');
          ...
          return new Ouro.Error('invalid_join', 'The `' + this.meta.relations[join].via + '` via attribute for this one-to-one join was not found on the `' + this.name + '` model.');
        }
  ...
  joinMany: function(resolve, reject, joins, records) {
    ...
    for( var join in joins ) {
      ...
      if( joins[join]._type !== 'oneToOne' ) {
        ...
        //make sure the relationship is setup
        if( typeof this.meta.relations[join] !== 'object') {
          return reject(new Ouro.Error('invalid_join', 'The `' + join + '` relationship has not been defined.'));
        }
        ...
        //make sure the model exists
        if( typeof this.adapter.models[joinModel] !== 'object') {
          return reject(new Ouro.Error('invalid_join', 'The `' + joinModel + '` model specified in the the relationship has not be defined.'));
        }

        if( typeof this.adapter.models[joinModel].meta.relations !== 'object' ||
          typeof this.adapter.models[joinModel].meta.relations[this.name] !== 'object'  ) {
            return reject(new Ouro.Error('invalid_join', 'The `' + join + '` the opposite site of the relationship has not been defined.'));
        }

        //determine type of many join
        if( typeof this.meta.relations[join].through === 'string' ) {

          //many to many relationships using through queries require both sides of the relationship to set the via property
          if( typeof this.meta.relations[join].via !== 'string' && typeof this.adapter.models[joinModel].meta.relations[this.name].via !== 'string' ) {
            return reject(new Ouro.Error('invalid_join', 'The `' + join + '` relationship uses a through table but both sides need to set the via property.'));
          } else {
            joins[join]._type = 'manyToMany';
          }

        } else if( typeof this.meta.relations[join].via === 'string' && typeof this.adapter.models[joinModel].meta.relations[this.name].via === 'string' ) {

          //one to many relationships need to determine the dominant side of the join using the via property
          return reject(new Ouro.Error('invalid_join', 'The `' + join + '` relationship does not use a through table, so only the dominant side should specify the via property.'));

        } else if( typeof this.adapter.models[joinModel].meta.relations[this.name].via === 'string' ) {

          joins[join]._type = 'oneToMany';

        } else {

          return reject(new Ouro.Error('invalid_join', 'The `' + join + '` relationship is not configured correctly.'));
        }
        ...
        if (joins[join].select) {
          ...
          if( badFields.length > 0 ) {
            ...
            return reject(new Ouro.Error('invalid_attribute', 'There are invalid attributes in your `' + join + '` join select list.', {details: details}));
          }
        ...
        if( joins[join]._type === 'manyToMany' ) {
          ...
          joinFields.push(this.meta.relations[join].through + '.' + this.meta.relations[join].via + ' as related::' + join + '::id');

          //setup our many to many query
          var query = this.db();
          query.select(joinFields);
          query.from(joinModel);
          query.innerJoin(this.meta.relations[join].through, this.meta.relations[join].through + '.' + this.adapter.models[joinModel].meta.relations[this.name].via, '=', joinModel + '.id');
          query.whereIn(this.meta.relations[join].through + '.' + this.meta.relations[join].via, recordIds);
          ...
          //add relationship id so we can properly add it to the appropriate record
          joinFields.push(joinModel + '.' + this.adapter.models[joinModel].meta.relations[this.name].via + ' as related::' + joinModel + '::id');

          //setup our one to many query
          var query = this.db();
          query.select(joinFields);
          query.from(joinModel);
          query.whereIn(joinModel + '.' + this.adapter.models[joinModel].meta.relations[this.name].via, recordIds);
  ...
  processRecord: function(record) {
    ...
    for( var field in record ) {
      ...
      if( field.indexOf('::') > -1 ) {
        ...
        //console.log(field, self.meta.attributes[field]);

        //translate json to objects
        if( self.meta.attributes[field].type === 'json' || self.meta.attributes[field].type === 'object' ) {
  ...
  processAttributes: function(attributes) {
    ...
    return _.mapObject(attributes, function(value, attribute) {
      ...
      if( (self.meta.attributes[attribute].type === 'json' || self.meta.attributes[attribute].type === 'object')  && typeof value === 'object' ) {

        //attempt to parse json
        try {
          value = JSON.stringify(value);
        } catch(err) {
          throw new Ouro.Error('invalid_json', 'The `' + attribute + '` attribute is set to json/object, and the object passed failed to convert to json.');
        }
  ...
  filter: function(query, table, filter, or) {
    ...
    for( var attribute in filter ) {
      ...
      if( !this.meta.attributes[attribute] && attribute !== 'or' ) {
        return new Ouro.Error('invalid_attribute', 'The `' + attribute + '` attribute does not exist on the ' + self.name + ' model.');
      }
      ...
      if( typeof filter[attribute] === 'number' || typeof filter[attribute] === 'string' ) {
        ...
        if( _.isArray(filter[attribute]) ) {
          ...
          if( invalidOperators.length > 0 ) {
            return new Ouro.Error('invalid_operator', 'The following invalid operators were used in your query: ' + invalidOperators.toString());
          }
